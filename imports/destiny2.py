...
credentials = authenticate(
    OAUTH_FILENAME, APP_KEY, APP_CLIENT_ID, APP_CLIENT_SECRET, args.reauth
)
    ...
    try:
        logger.info(
            "Looking at membership for {}".format(
                MEMBERSHIP_TYPES[member_data["membershipType"]]
            )
        )
    ...
    for character_id in character_list:
        ...
        for instance in activities["activities"]:
            ...
            activity = destinyEntity(
                "DestinyActivityDefinition", instance["activityDetails"]["referenceId"]
            )
            ...
            logger.info(
                "Completed instance {name} at {period}".format(
            ...
            item = {"instance_info": instance, "activity_info": activity}
