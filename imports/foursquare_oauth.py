...
if not CLIENT_ID:
    ...
    print("=== Copy the CODE in the URL that results and run %s %s [code]" % (
        sys.argv[0], sys.argv[1]))

    print("https://foursquare.com/oauth2/authenticate?client_id=%s&response_type=code&redirect_uri=%s" %
    ...
    url = "https://foursquare.com/oauth2/access_token?client_id=%s&client_secret=%s&grant_type=authorization_code&redirect_uri=%s&code=%s" % (
        CLIENT_ID, CLIENT_SECRET, CALLBACK_URL, FS_ACCESS_CODE)
    web = Http()
