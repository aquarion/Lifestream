...
if not client_token:
    ...
    client_token = oauth.fetch_token(
        token_url=BASE_OAUTH_URL + "/oauth/token",
        client_id=APP_KEY,
    ...
    def get_profile(self):
        try:
            ...
            if oauthfile.st_mtime > cachefile.st_mtime or profile_age > 2:
                ...
                logger.info(
                    "Oauth: {} Cache: {}, not refreshing".format(
                        datetime.fromtimestamp(oauthfile.st_mtime),
                        datetime.fromtimestamp(cachefile.st_mtime),
                    )
                )
...
if datetime.now() >= creation_date + expiry:
    logger.info(
        "User access token is {} days old, please run with --reauth to refresh".format(
            delta.days
        )
    )
else:
