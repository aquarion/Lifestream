...
lifestream.arguments.add_argument(
    "--all", required=False, help="Get all posts", default=False, action="store_true"
)
...
OAUTH_FILENAME = "%s/facebook.oauth" % (lifestream.get_credentials_dir())
...
def authenticate(OAUTH_FILENAME, appid, secret, force_reauth=False):
    ...
    request_token_url = (
        "https://www.facebook.com/dialog/oauth?client_id=%s&redirect_uri=%s&response_type=token&scope=user_posts"
        % (appid, redirect_uri)
    )
