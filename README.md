...
Production runs **MariaDB 10.11**. The baseline migration's `TEXT`/`BLOB`
columns (`lifestream.title`/`image`/`fulldata_json`,
`lifestream_locations.fulldata_json`) use column-level `DEFAULT` values,
which requires **MariaDB ≥ 10.2.1** or **MySQL ≥ 8.0.13** — plain MySQL
before that doesn't support it and `alembic upgrade head` will fail on an
older server.
